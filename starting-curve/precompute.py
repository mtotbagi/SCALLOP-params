...
for extdeg in extToFac.keys():
    if(extdeg == 1):
        ...
        for D in extToFac[extdeg]:
            ...
            Q = Ebig(xQ, yQ)
            e = time.time()
            print(f"Basis for torsion {D} found (with pari) in: {e-s}")

            #P, Q = TorsionBasis(Ebig, D, xOnly=1, E0=E0)
            PmQ = P-Q
            xP, xQ, xPmQ = xPoint(P.xy()[0], Ebig), xPoint(Q.xy()[0], Ebig), xPoint(PmQ.xy()[0], Ebig)
            facToBasis[D] = [xP, xQ, xPmQ]     
            # Compute basis action
            Basis = P, Q   
            facToAction[D] = BasisAct(Basis, D)
        continue
    D = lcm(extToFac[extdeg])

    print("Finding torsion", factor(D))
    #print(f"Expected cost: {cost(extdeg)}")
    s = time.time()
    i=1
    while True:
            print("aaaa")
    ...
    e = time.time()
    print(f"Basis for torsion {D} found (with pari) in: {e-s}")
    #assert has_order(Pbig, D)
    #assert has_order(Qbig, D)
    #assert is_independent(Pbig, Qbig, D)
    #Pbig, Qbig = TorsionBasis(Ebig, D, xOnly = 1, E0 = E0)
